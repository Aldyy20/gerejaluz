...
if (isset($_GET['id'])) {
    ...
    $qry = mysqli_query($konek, "SELECT foto FROM tbl_mjh WHERE kode='$id'");

    if ($qry && mysqli_num_rows($qry) > 0) {
        $data = mysqli_fetch_array($qry);

        $lokasi = $data['foto']; // Sesuaikan dengan nama kolom yang menyimpan nama file
        $hapus_file = "../img/mjh/$lokasi"; // Sesuaikan dengan path file yang benar
        ...
        mysqli_query($konek, "DELETE FROM tbl_mjh WHERE kode='$id'");

        // Redirect ke halaman yang sesuai
        header('Location: mjh_tampil.php'); // Sesuaikan dengan halaman yang diinginkan
